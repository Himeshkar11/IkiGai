  ...
  <li className="food-entry">
      ...
      <div className="food-entry-body">
        ...
        {editing ? (
          <div className="food-entry-edit">
            ...
            <button type="button" className="btn primary small" onClick={onSaveEdit}>Save</button>
          </div>
        ) : (
          <div className="food-entry-meta">
            Protein {Math.round(item.protein || 0)}g · Carbs {Math.round(item.carbs || 0)}g · Fat {Math.round(item.fat || 0)}g
          </div>
        )}
      </div>
    </div>
    ...
    {!editing && (
      <div className="food-entry-actions">
        <button type="button" className="icon-menu-btn" aria-label="Entry options" onClick={onToggleMenu}>⋮</button>
        {menuOpen && (
...
const FoodPage = () => {
  ...
  const saveEdit = async (item, meal) => {
    if (!foodLog?._id) return;
    try {
      const updated = await foodService.updateMealItem(foodLog._id, meal, item._id, { quantity: editQty });
      setFoodLog(updated);
    } catch (e) {
    ...
    <div className="page-card food-page">
      ...
      <div className="food-section">
        ...
        <div className="nutrition-stats">
          {statFields.map((f) => (
            <div key={f.key} className="nutrition-stat">
              <div className="nutrition-stat-value">{Math.round(totals[f.key] || 0)}{f.unit === 'g' ? 'g' : ''}</div>
              <div className="nutrition-stat-label">{f.label}{f.unit === 'kcal' ? ' (kcal)' : ''}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <>
          <div className="food-section add-food-section">
            {formOpenFor === 'global' ? (
              ...
              <button className="add-food-cta" onClick={() => openForm(null)}>+ Add Food</button>
            )}
          </div>
          ...
          <div className="meal-list">
            {meals.map((m) => {
              ...
              const formOpenHere = formOpenFor === m.key;
              return (
                <div key={m.key} className="meal-card-v2">
                  <div className="meal-card-header">
                    <h3><span aria-hidden>{m.icon}</span> {m.label}</h3>
                    {!formOpenHere && (
                      <button className="add-mini" onClick={() => openForm(m.key)}>+ Add</button>
                    )}
                  </div>

                  {formOpenHere && renderForm(m.key)}

                  {items.length === 0 ? (
                    <div className="meal-empty">No food logged yet</div>
                  ) : (
                    <ul className="food-entries">
                      {items.map((it) => (
                        <FoodEntryRow
                          ...
                          item={it}
                          icon={m.icon}
                          editing={editingItemId === it._id}
                          editQty={editQty}
                          onEditQtyChange={setEditQty}
                          menuOpen={openMenuItemId === it._id}
                          onToggleMenu={() => setOpenMenuItemId((prev) => (prev === it._id ? null : it._id))}
                          onStartEdit={() => startEdit(it)}
                          onCancelEdit={() => setEditingItemId(null)}
                          onSaveEdit={() => saveEdit(it, m.key)}
                          onDelete={() => handleDelete(it, m.key)}
                        />
                      ))}
                    </ul>
                  )}
